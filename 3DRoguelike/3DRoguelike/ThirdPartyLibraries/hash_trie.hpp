...
class node {
 ...
 protected:
    ...
    void dbgPrintRefs(char const* type, size_t refCount) const {
        std::cout << m_id << " @ 0x" << std::hex << (unsigned long)this << " " << type << " " << refCount << "/ " << dbg_get_total_refs()
                  << std::endl;
    }
...
class leaf_node : public node {  // NOLINT
    friend struct std::default_delete<leaf_node>;
    ...
    ~leaf_node() {
        for (size_t i = 0; i < m_size; ++i) m_values[i].~T();
    }
 ...
 public:
    ...
    auto with_appended_value(U&& newValue) const {
        ...
        for (size_t i = 0; i < m_size; ++i) new (&newLeaf->m_values[i]) T(m_values[i]);
    ...
    auto find(T const& value) const -> T const* {
        for (size_t i = 0; i < m_size; ++i)
            if (m_values[i] == value) return &m_values[i];
        return nullptr;
    }
...
class branch_node : public node {  // NOLINT
    friend struct std::default_delete<branch_node>;
    ...
    explicit branch_node(size_t size, size_t bitmap)  // NOLINT
        : node(node_type::branch), m_size(size), m_bitmap(bitmap) {
    }
 ...
 public:
    ...
    static auto create_pair(sparse_index index1, leaf_node<T> const* leaf1, sparse_index index2, leaf_node<T> const* leaf2)
        -> std::unique_ptr<branch_node> {
    ...
    auto get_at(sparse_index sparseIndex) const -> node const* {
        if ((m_bitmap & sparseIndex.bit_position()) == 0) return nullptr;
...
class path {
 ...
 public:
    ...
    auto last_branch() const {
        return m_lastBranch;
    }
    auto leaf() const {
        return m_leaf;
    }
    auto whole_hash() const {
        return m_chunkedHash.hash;
    }
    auto hash_chunk() const {
        return m_chunkedHash.chunk;
    }
    auto chunked_hash() const {
        return m_chunkedHash;
    }
...
template <typename T>
auto extend(detail::chunked_hash existingHash, leaf_node<T> const* existingLeaf, detail::chunked_hash newHash,
    ...
    if (existingHash.chunk == newHash.chunk) {
        ...
        auto newBranch = branch_node<T>::create_pair(sparse_index(existingHash.chunk), existingLeaf, sparse_index(newHash.chunk), newLeaf.get());
        newLeaf.release();
        addref(existingLeaf);
        return newBranch;
    }
...
static auto add_value_at_leaf(path<T> const& path, U&& value) -> branch_node<T> const* {
    ...
    if (existingHash.hash == path.whole_hash()) {
        ...
        auto newBranch = path.last_branch()->with_replaced(sparse_index(path.hash_chunk()), newLeaf.get());
    ...
    // Different hash, so add branches to the point they diverge
        ...
        auto newChildBranch =
            extend(existingHash + 1, existingLeaf, path.chunked_hash() + 1, leaf_node<T>::create(std::forward<U>(value), path.whole_hash()));
        auto newBranch = path.last_branch()->with_replaced(sparse_index(path.hash_chunk()), newChildBranch.get());
        auto newRoot = path.rewrite(newBranch.get());
        newChildBranch.release();
        newBranch.release();
        return newRoot;
...
static auto inserted(branch_node<T> const* root, U&& value) -> branch_node<T> const* {
    ...
    return path.leaf() ? add_value_at_leaf(path, std::forward<U>(value))
...
class hash_trie {
 ...
 public:
    ...
    auto data() const -> hash_trie_data<T> const& {
        return m_data;
    }
    auto data() -> hash_trie_data<T>& {
        return m_data;
    }
};
...
class shared_hash_trie {  // NOLINT
 ...
 public:
    ...
    auto reset(hash_trie_data<T>& originalData, hash_trie_data<T>& newData) -> bool {
        if (!m_data.compare_exchange_strong(originalData, newData, std::memory_order_release, std::memory_order_relaxed)) return false;
...
class hash_trie_transaction {
 ...
 public:
    explicit hash_trie_transaction(shared_hash_trie<T>& shared) : m_baseData(shared.data()), m_shared(shared) {
    ...
    void update_with(L const& updateTask) {
        while (true) {
            ...
            if (copy.data().m_root == m_baseData.m_root) break;

            // try to commit, and if successful we're done
            if (try_commit(copy)) break;
