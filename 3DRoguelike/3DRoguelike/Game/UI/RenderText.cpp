...
void TextRenderer::LoadFont(const std::string& file) {
    FT_Library ft;
...
void TextRenderer::RenderText(const std::string& text, glm::vec2 coords, float scale, glm::vec3 color) {
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    // activate corresponding render state
    shader.use();
    shader.setMat4("projection", Assets::Get().orthogonalProjection);
