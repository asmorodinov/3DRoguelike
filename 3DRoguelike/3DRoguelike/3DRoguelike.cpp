...
int main() {
    ...
    LOG_ASSERT(n == 6);        // 6 points have been inserted
    LOG_ASSERT(T.is_valid());  // checking validity of T
    Locate_type lt;
    int li, lj;
    Point p(0, 0, 0);
    Cell_handle c = T.locate(p, lt, li, lj);
    // p is the vertex of c of index li :
    LOG_ASSERT(lt == Triangulation::VERTEX);
    LOG_ASSERT(c->vertex(li)->point() == p);
    ...
    int nli;
    LOG_ASSERT(nc->has_vertex(v, nli));
